...
public class lsb {
	...
	private void initialize() {
		...
		btnProcessextract.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//System.out.println(extractInfo(img, txtpassextract.getText()));
				lbinfo.setText(extractInfo(img, txtpassextract.getText()));
			}
		});
	...
	public BufferedImage hideInfo(BufferedImage img, String info, String pass){
		...
		int lengthpass=arrbnpass.length;
		int[] arrlengthpass=chuyensovenhiphan(arrbnpass.length);
		int lengthinfo=arrbninfo.length;
		int[] length=chuyensovenhiphan(arrbninfo.length);
		int x=0;
		int y=0;
		// giau chieu dai pass
		for(int i=0;i<arrlengthpass.length;i++){
			int rgb = img.getRGB(y, x);
			if(arrlengthpass[i] == 0){
				rgb = rgb & 0xFFFEFFFF;
			}
			else {
				rgb = (rgb | 0x00010000);
...
//		//giau thong tin
	...
	public String extractInfo(BufferedImage img, String pass){
		int width = img.getWidth();
		int height = img.getHeight();
		int x=0;
		int y=0;
		int[] arrlengthpass= new int[24];
		//lay chieu dai mat khau
		for(int i=0;i<24;i++){
			Color c = new Color(img.getRGB(y, x));
			arrlengthpass[i]=c.getRed()%2;
			x=(x+1)%width;
			y=y+(x/width);
		}
		
		int lengthpass=chuyennhiphanvethapphan(arrlengthpass);
		System.out.print(lengthpass);
		int[] arrpass=new int[lengthpass];
		for(int i=0;i<lengthpass;i++){
			Color c = new Color(img.getRGB(y, x));
			arrpass[i]=c.getRed()%2;
			x=(x+1)%width;
			y=y+(x/width);
		}
		if(!chuyennhiphanvechuoi(arrpass).equals(pass)){
...
//		}
		...
		return chuyennhiphanvechuoi(arrinfo);
